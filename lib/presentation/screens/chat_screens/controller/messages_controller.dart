...
class MessageController{
  ...
  static Future<List<dynamic>?> getMessages({bool dioLoading = true, String? msgId}) async{
    String? token;
...
static Future<bool?> addMessages(String text,String chatId,String senderId)async{
  try {
    String? token;
    SharedPreferences prefs = await SharedPreferences.getInstance();
    token = prefs.getString('token');
    Response res = await _dioClient.post(
      'api/v1/messages',
       token,
       body: ChatBody.messageMap( text: text, chatId: chatId, senderId: senderId))as Response;
